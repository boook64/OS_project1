...
// #define UNIT_TIME { volatile unsigned long x; for (x = 0; x < 100UL; x ++); }
...
void StartRunProcess(struct Process *now) {
	...
	if (pid == 0) {
		...
		// syscall(GET_CLOCK, &st);
		for (int i = 0; i < now->t; ++ i) UNIT_TIME;
		// syscall(GET_CLOCK, &ed);
		// syscall(PRINTER, st.tv_sec, st.tv_nsec, ed.tv_sec, ed.tv_nsec, getpid());
		exit(0);
	}
	else {
		now->pid = pid;
...
void SchedulerRR(struct Process *ps, int n) {
	...
	// for (int i = 0; i < n; ++ i) printf("[DEBUG] prob %s %d %d\n", ps[i].name, ps[i].s, ps[i].t);
	for (int times = 0; ; ++ times) {
		// Finish runnning
		// if (times == 3000) break;
		// if (times % 100 == 0) {printf("[DEBUG] times: %d\n", times);fflush(stdout);}
		// if (times % 800 == 0)
		// 	{for (int j = 0; j < n; ++ j) {
		// 		{printf("%d ", ps[j].t);fflush(stdout);}
		// 	}
		// 		puts("");}
...
void SchedulerSJF(struct Process *ps, int n) {
	int run_id = 0, next_job = 0, finish_cnt = 0;
	int status[n]; // -1: finish, 0: unbegin, 1: begin
	memset(status, 0, sizeof status);
	for (int times = 0; ; ++ times) {
		if (ps[run_id].isrun != 0 && ps[run_id].t == 0) {
			ps[run_id].isrun = 0, wait(0);
			status[run_id] = -1;
			++ finish_cnt;
		...
		if (ps[run_id].isrun == 0) {
			for (int i = 0; i < n; ++ i) 
				if (status[i] == 1 && status[run_id] != 1) run_id = i;
			for (int i = 0; i < n; ++ i) 
				if (status[i] == 1 && ps[i].t < ps[run_id].t) run_id = i;
...
void SchedulerPSJF(struct Process *ps, int n) {
	...
	for (int times = 0; ; ++ times) {
		...
		while (next_job < n && ps[next_job].s <= times) {
			...
			if (ps[run_id].isrun != 0 && ps[next_job].t < ps[run_id].t) {
		...
		UNIT_TIME;
		if (ps[run_id].isrun != 0) -- ps[run_id].t;
	}
}
